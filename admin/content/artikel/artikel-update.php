...
$hasil = mysqli_fetch_array($sql);
	...
	$headline = mysqli_real_escape_string($koneksi, trim(nl2br(strip_tags($hasil['headline']))));
	$headline2 = substr($headline,0,10);
	$isi = mysqli_real_escape_string($koneksi, trim(nl2br(strip_tags($hasil['isi']))));
	$isi2 = $isi;
	$isi3 = str_replace('\r\n', "<br />", $isi);
	$isi4 = str_replace('<br />', "  ", $isi);
			...
			<input type="hidden" name="getgambar" value="<?php echo $gambar; ?>" class="input-xlarge">
				<table class="table table-striped">
						...
						<tr>
								...
								?>
								</select> <br /><q>Kategori awal</q> : <b><i>
								<?php
						...
						<tr>
								<td>Isi Artikel</td>
								<td>:</td>
								<td><textarea name="isi" class="input-xlarge"><?php echo $isi4; ?></textarea></td>
								</tr>
						<tr>
								<td>Pengirim</td>
								<td>:</td>
								<td><input type="text" name="pengirim" class="input-xlarge" value="<?php echo $pengirim; ?>"></td>
								</tr>
						<tr>
								<td>Gambar</td>
								<td>:</td>
