<div class="">
	<h2 style="color: #FFFFFF;">&nbsp;<i class="icon icon-tags" style="-webkit-transform:scale(1.8); -moz-transform:scale(1.8); -o-transfrom:scale(1.8); transform:scale(1.8); margin-top: 10px;"></i>&nbsp; Kategori :</h2>
		
<?php
require("koneksi.php");

$select = "SELECT id_kategori FROM kategori ORDER BY id_kategori ASC";
$query = mysqli_query($koneksi, $select);
$r = mysqli_fetch_array($query);
$kategori = mysqli_real_escape_string($koneksi, trim($r['id_kategori']));

echo "<span><h4 style='color: #FFFFFF; padding-bottom: 10px;'>";

	$select1 = "SELECT * FROM kategori WHERE LOWER(nm_kategori)='portfolio'";
	...
	$nm_kategori1 = mysqli_real_escape_string($koneksi, trim(ucwords($r1['nm_kategori'])));
		...
		echo "<a href='#' style='color: #FFFFFF;'>".$nm_kategori1."</a>";
...
while($r = mysqli_fetch_array($query)) {
	...
	$judul = mysqli_real_escape_string($koneksi, trim($r['judul']));
		...
		echo "<h5 style='display: block; box-shadow: 1px 3px 5px 1px rgba(0,0,0,0.2); padding: 6px; border-radius: 5px;'>
			<a style='color: #313131;' href='index.php?menu=portfolio-detail&id_artikel=$id_artikel'>$judul</a>
			</h5>
